...
with open('feed.txt', 'r') as file1:
    count=0
    for line in file1:
        count+=1
        split_line = line.split(',')
        mpd_url = split_line[0].strip()
        license = split_line[1].strip()
        ...
        for key in keys:
            ...
            title = license.split("=")[-2].replace("&build", "").title()
            path = r'KEYS\\'
            ...
            def download_drm(mpd_url):
                #reso = args.quality
                #Parses the mpd url to get the height resolution and use as input
                reso = mpd_url.partition("manifest.")[2].partition('x')[-3]
                print(reso)
                settings_video = 'res="{res}*":codec=avc1:for=best'.format(res=reso)
                download = ([m3u8re, mpd_url, '--save-name', title.replace(' ', '.').replace('-', '.'),
                             '--tmp-dir', "cache/", '--save-dir', "cache/", '--key-text-file',
                             path + 'IWANTTFC_KEYS.txt',
                             '--sub-format', "SRT",
                             '--write-meta-json', "false", '-sv', settings_video, '-sa', "all", '-ss', "all",
            ...
            mediainfo_output_audio = subprocess.Popen([mediainfo, '--Output=JSON', '-f', out_audio],
                                                      stdout=subprocess.PIPE)
            mediainfo_json_audio = json.load(mediainfo_output_audio.stdout)
            mediainfo_audio = mediainfo_json_audio

            mediainfo_output_video = subprocess.Popen([mediainfo, '--Output=JSON', '-f', out_video],
                                                      stdout=subprocess.PIPE)
            ...
            if video_res == "2160" or video_resw == "3840":
                vid_res = "2160p"
            elif video_res == "1152" or video_resw == "2184":
                vid_res = "2160p"
                ...
                vid_res = "540p"
            elif video_res == "386" or video_resw == "682":
                vid_res = "480p"
            elif video_res == "480":
                vid_res = "480p"
            elif video_res == "360":
            ...
            output = "{}.WEB-DL.{}-{}".format(title.replace(' ', '.'), vid_res,args.group)
            SUBS = ""
            ...
            print(f"{GREEN}[+] Done with Line#{count} {RESET}")
            #sys.exit()
